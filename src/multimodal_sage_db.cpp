...
MultimodalSageDB::MultimodalSageDB(const MultimodalConfig& config) 
    : SageDB(config.base_config), multimodal_config_(config) {
    
    if (!validate_multimodal_config()) {
        throw SageDBException("Invalid multimodal configuration");
    }
...
VectorId MultimodalSageDB::add_multimodal(const MultimodalData& data) {
    ...
    // Add the fused vector to the base SAGE DB
    return SageDB::add(fused_vector, data.global_metadata);
}

VectorId MultimodalSageDB::add_multimodal(
...
std::vector<QueryResult> MultimodalSageDB::search_multimodal(
    ...
    return SageDB::search(query_vector, base_params);
}

void MultimodalSageDB::validate_multimodal_data(const MultimodalData& data) const {
...
Vector MultimodalSageDB::perform_fusion(
...
Vector MultimodalSageDB::build_query_vector(
    const std::unordered_map<ModalityType, ModalData>& query_modalities,
    const MultimodalSearchParams& params) const {
...
void MultimodalSageDB::register_modality_processor(ModalityType type,
                                                  std::shared_ptr<ModalityProcessor> processor) {
    modality_manager_->register_processor(type, processor);
}

void MultimodalSageDB::register_fusion_strategy(FusionStrategy strategy,
                                               std::shared_ptr<FusionStrategyInterface> impl) {
    fusion_engine_->register_strategy(strategy, impl);
}

void MultimodalSageDB::update_fusion_params(const FusionParams& params) {
    multimodal_config_.default_fusion_params = params;
}

const FusionParams& MultimodalSageDB::get_fusion_params() const {
    return multimodal_config_.default_fusion_params;
}

std::vector<ModalityType> MultimodalSageDB::get_supported_modalities() const {
    return modality_manager_->get_supported_modalities();
}

std::vector<FusionStrategy> MultimodalSageDB::get_supported_fusion_strategies() const {
    return fusion_engine_->get_supported_strategies();
}

bool MultimodalSageDB::validate_multimodal_config() const {
...
void MultimodalSageDB::register_default_fusion_strategies() {
...
std::unique_ptr<MultimodalSageDB> MultimodalSageDBFactory::create_text_image_db(
    ...
    return std::make_unique<MultimodalSageDB>(config);
}

std::unique_ptr<MultimodalSageDB> MultimodalSageDBFactory::create_audio_visual_db(
    const DatabaseConfig& base_config) {
