# SAGE DB 使用模式指南（Standalone / BYO-Embedding / Plugin / Service）

本指南阐述 SAGE DB（下称 sagedb）的四种典型使用方式，帮助你在不同场景中做出合适选择，并快速落地：
- Standalone（独立）：sagedb-core + 可选内置处理器（builtin）
- BYO-Embedding（仅向量）：只用 sagedb-core，外部自行生成 embeddings
- Plugin（插件注入）：sagedb-core + 运行期加载的第三方/上层项目插件（如 SAGE 提供）
- Service（微服务）：把 sagedb 以 REST/gRPC 的形式对外提供

> 术语约定：
> - core：sagedb 核心库（索引/检索/融合/元数据 + 抽象接口，如 ModalityProcessor）。
...
- `sage_db::MultimodalSageDB`：多模态 DB 入口，负责融合、索引、检索、元数据等。
- `sage_db::ModalityProcessor`：模态处理器抽象（把原始 bytes 处理成 embedding 向量）。
- `sage_db::ModalityManager`：处理器注册与调度（`register_modality_processor`）。
- `sage_db::FusionStrategy` 与 `FusionEngine`：多模态融合策略与执行（拼接、加权、注意力…）。
- `sage_db::ModalData`：承载某一模态的输入（可直接是 embedding 或原始数据）。

> 重要：当前实现中，若你传入的 `ModalData` 已含 `embedding`，sagedb 会直接使用该向量；只有当你选择“处理原始数据”这条路径并注册了对应处理器时，才会调用 `ModalityProcessor` 去生成 embedding。

---

## 1) Standalone（独立 + builtin）

- 组成：`sagedb-core` + `sagedb-builtin`（可选构建）。
...
#include "sage_db/modality_processors.h" // 声明在 include 中；具体实现由 builtin 或外部提供
...
注意事项：
- builtin 应作为可选组件构建（例如 CMake: `BUILD_SAGEDB_BUILTIN`）。
...
- 组成：仅 `sagedb-core`。
- 适用：把 embedding 生成留给上层项目（如 SAGE），sagedb 专注索引/检索/融合/元数据。
- 优点：无循环依赖、最小耦合、最稳定；sagedb 可作为独立第三方库发布。
...
注意事项：
- 若传入 `ModalData.embedding` 为空，sagedb 当前不会自动生成 embedding；需自行注册处理器或走 Plugin/Standalone。

---

## 3) Plugin（插件注入）

- 组成：`sagedb-core` + 插件动态库（由上层项目或第三方提供，如 SAGE）。
...
```cpp
extern "C" void register_sagedb_plugin(sage_db::MultimodalSageDB* db) {
    using namespace sage_db;
    auto text = std::make_shared<TextModalityProcessor>(TextModalityProcessor::TextConfig{});
    db->register_modality_processor(ModalityType::TEXT, text);
}
...
- 组成：将 `sagedb-core` 打包为独立进程服务（REST/gRPC）。
...
- Q：独立使用 sagedb 时能否不依赖任何模型？
  - A：可以，选择 BYO-Embedding（只传入 embedding），或启用内置（builtin）做本地演示。

- Q：如何避免与上层项目（如 SAGE）产生循环依赖？
  - A：通过 BYO-Embedding 或 Plugin。sagedb 只提供接口，SAGE 在运行时注入实现（注册处理器）。
...
- 多团队/跨语言/需要横向扩展：将 sagedb 以 Service 形态部署。
